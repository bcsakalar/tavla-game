...
> ✅ **Zaten mevcut!** Wildcard sertifika (`*.berkecansakalar.com`) VPS'te storyforge ile birlikte kurulmuş durumda.
> Ekstra bir şey yapmana gerek yok.

Sertifika dosyaları:
```
/etc/ssl/cloudflare/storyforge.pem     ← Origin Certificate
/etc/ssl/cloudflare/storyforge.key     ← Private Key
```

Doğrulama:
```bash
# Dosyaların var olduğunu kontrol et
ls -la /etc/ssl/cloudflare/storyforge.*
```
...
```
/etc/ssl/cloudflare/
├── storyforge.pem      ← Cloudflare Origin Certificate (wildcard, zaten mevcut)
└── storyforge.key      ← Cloudflare Private Key (zaten mevcut)
...
# 2. SSL sertifika (zaten mevcut, atlayabilirsin)
ls -la /etc/ssl/cloudflare/storyforge.*   # Wildcard cert kontrol
